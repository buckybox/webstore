...
describe Cart do
  let(:persistence_class) { double("persistence_class") } # rubocop:disable RSpec/VerifiedDoubles
  let(:persistence)       { double("persistence", cart: cart) } # rubocop:disable RSpec/VerifiedDoubles
  let(:args)              { { webstore_id: 1, persistence_class: persistence_class } }
  let(:cart)              { described_class.new(args) }
  ...
  describe ".find" do
    context "when a cart is found" do
      it "returns restored cart" do
        ...
        expect(described_class.find(1, persistence_class).new?).to be false
      end
    end
    ...
    context "when a cart is not found" do
      it "returns nil" do
        ...
        expect(described_class.find(1, persistence_class)).to be_nil
      end
    end
  end
  ...
  describe "#new?" do
    it "is considered new if the cart does not have an true" do
      cart_without_id = described_class.new(args)
      expect(cart_without_id.new?).to be true
    end

    it "is not considered new if the cart has an true" do
      cart_without_id = described_class.new(args.merge(id: 1))
  ...
  describe "#==" do
    context "a cart is new" do
      ...
      it "returns false if the carts are diffent objects" do
        cart2 = described_class.new(args)
    ...
    context "a cart has been saved" do
      ...
      it "returns true if the carts have the same id" do
        cart2 = described_class.new(args.merge(id: 1))
        expect(cart1).to eq(cart2)
      end

      it "returns false if the carts have a different id" do
        cart2 = described_class.new(args.merge(id: 2))
        expect(cart1).not_to eq(cart2)
      end
    end
  end
